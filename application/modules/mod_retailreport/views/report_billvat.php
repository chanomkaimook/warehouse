<?php
	...
	$texttopic = "ใบส่งของ/ใบกำกับภาษี";
	$texttopic2 = "ใบเสร็จรับเงิน";
	$textvoice = "เลขที่ ";
	...
	foreach($query as $row => $val){
		...
		if($val->bill_tax == 0 || $val->bill_tax == null){
			...
			$taxprice = $val->bill_tax;
		}
		
		//	PARCEL
		if($val->bill_parcel == 0 || $val->bill_parcel == null){
			$parcel = 0;
		}else{
		...
		//	SHOR
		if($val->bill_shor == 0 || $val->bill_shor == null){
			$shor = 0;
		}else{
			$shor = $val->bill_shor;
		}
		...
		//	DELIVERY
		if($val->bill_delivery == 0 || $val->bill_delivery == null){
			$delivery = 0;
		}else{
		...
		// echo htmlspecialchars($renew_address)."---<br>";exit;
		// 
		
		//
		//	discout will value lessthan 0
		$decreasediscount = $val->bill_discount;
		...
		$write_array[] = array(
							...
							"parcel"		=> $parcel,
							"delivery"		=> $delivery,
							"discount"		=> $decreasediscount,
							"netprice"		=> $val->bill_nettotal,
							"date"			=> $val->bill_datetime,
							"dateslip"		=> $val->bill_datetimeslip
						);
	}
	// die();
	#	group date order
	...
	#	
	#	creat text sign customer
	$listText = new \PhpOffice\PhpSpreadsheet\RichText\RichText();
	$listText->createText($sign_customer);
	$textSign = $listText->createTextRun($write_array[$key]['name']);
	$textSign->getFont()->setName($fontfamily);
	$textSign->getFont()->setSize($fontsize);
	#
	#	insert text
	$spreadsheet->getActiveSheet()->getCell($cl_1_s.$nextrow_sign)->setValue($listText);
	...
	$drawing->setPath("asset/images/back/sign/sak_burger.jpg");
